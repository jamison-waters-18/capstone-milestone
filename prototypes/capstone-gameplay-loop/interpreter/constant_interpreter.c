...
void* exec_long_rest(GPtrArray* args, returnType* return_type) {
	if (args->len) {
		*return_type = RTRN_FAIL;
		return FAILED_FUNC_EXEC;
	}
...
void* exec_short_rest(GPtrArray* args, returnType* return_type) {
	if (args->len) {
		*return_type = RTRN_FAIL;
		return FAILED_FUNC_EXEC;
	}
...
funcType find_const_func(char* func_name, returnType* return_type) {
	...
	if (!strcmp(func_name, "long_rest")) {
		*return_type = RTRN_PROCESS_TIMING;
		return exec_long_rest;
	}
	if (!strcmp(func_name, "short_rest")) {
		*return_type = RTRN_PROCESS_TIMING;
		return exec_short_rest;
	}
